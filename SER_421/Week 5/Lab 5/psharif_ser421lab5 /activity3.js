...
function greeting (){
    ...
    if(localStorage.getItem(userName)){
        conversation="";
        var pastTalk=localStorage.getItem(userName);
        pastTalk.replace("Hi I'm Eliza, what's your name?", "");
        pastTalk=pastTalk.substring(0,pastTalk.lastIndexOf("<br>"));
        var oldJSONKeys=localStorage.getItem(userName + 'key');
        var oldKeys=JSON.parse(oldJSONKeys);
        used=oldKeys.slice(0);
        output(pastTalk);
    ...
    if(!localStorage.getItem(userName)){
        localStorage.setItem(userName,conversation);
        localStorage.setItem(userName + 'key',JSON.stringify(used));
    }
